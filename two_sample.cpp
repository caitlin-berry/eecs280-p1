...
vector<double> mean_diff_sampling_distribution(vector<double> data_A, vector<double> data_B) {
  ...
  for (int i = 0; i < 1000; ++i){
    ...
    vector dataAResample = bootstrap_resample(data_A, i);
    vector dataBResample = bootstrap_resample(data_B, i);
    //   2. Compute the difference in means between the resamples
    double meanDifference = mean(dataAResample) - mean(dataBResample);
    //   3. Add the computed value to a vector
    newV.push_back(meanDifference);
  }
  return newV;
}
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  // HINT: Use the percentile function as a helper to compute the lower and upper bounds.
  double midPoint = width / 2;
  double lower = 0.5 - midPoint;
  double upper = 0.5 + midPoint;
...
void two_sample_analysis(string file_name, string filter_column_name,
  ...
  auto [filter_data, data] = extract_columns(file_name, filter_column_name, data_column_name);
